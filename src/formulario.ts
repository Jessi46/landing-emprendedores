...
export class GestorContactos {
  ...
  #alEnviar(evento: Event): void {
    ...
    this.#leads.push(new Lead(this.#siguienteId++, nombre, correo, plan));
    this.#render();
    this.#form.reset();
  }
  ...
  #render(): void {
    ...
    this.#leads.forEach((lead) => {
      ...
      li.textContent = lead.presentar();   // usa el polimorfismo
  ...
  // Guardar en JSON
  guardarJSON(): void {
    const json = JSON.stringify(this.#leads, null, 2);
  ...
  cargarJSON(texto: string): void {
    const datos = JSON.parse(texto) as IContactoPlano[];
    this.#leads = datos.map(
      (d) => new Lead(d.id, d.nombre, d.correo, d.plan)
    );
    this.#render();
  }
}
